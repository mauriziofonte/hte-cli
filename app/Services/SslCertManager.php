...
/**
 * Generates self-signed SSL certificate scripts for local development.
 */
class SslCertManager implements SslCertManagerInterface
{
     ...
     * @param string $certsDir Directory for storing certificates.
     */
    public function __construct(FilesystemInterface $fs, $certsDir = '/etc/apache2/certs-selfsigned')
    {
        $this->fs = $fs;
    ...
    {
        // Validate domain to prevent shell injection in the generated script
        if (!preg_match('/^[a-zA-Z0-9._-]+$/', $domain)) {
            throw new \InvalidArgumentException("Invalid domain name: {$domain}");
        }

        // Ensure the certs directory exists
        if (!$this->fs->isDir($this->certsDir)) {
            $this->fs->makeDirectory($this->certsDir, 0755, true);
        }

        $ssldir = $this->certsDir;
